...
"""
Beacon v0.8 — Search Service: Kafka Consumer

Consumes events from "beacon.events.pages" topic and indexes
into Elasticsearch. Uses idempotency keys for at-least-once processing.

Chapter 8, Principle 5: "Events, not commands."
"""
...
class PageIndexer:
    ...
    def index_page(self, page_data: dict):
        logger.info("Indexing page %s: %s", page_data.get("id"), page_data.get("title"))
        # In production: use elasticsearch-py to index the document.

    def delete_page(self, page_id: int):
        logger.info("Deleting page %s from index", page_id)

    def handle_event(self, event_type: str, payload: dict):
        if event_type == "page.created" or event_type == "page.updated":
            self.index_page(payload)
        elif event_type == "page.deleted":
            self.delete_page(payload.get("page_id", 0))
        else:
