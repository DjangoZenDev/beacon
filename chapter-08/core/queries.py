"""Beacon v0.8 — Scatter-Gather Queries (carried from Ch6)."""
...
def global_search(query, org_ids, max_results=20):
    ...
    with ThreadPoolExecutor(max_workers=min(len(shards), 8)) as ex:
        ...
        for f in as_completed(futures):
            try:
                results.extend(f.result(timeout=5))
...
def _excerpt(body, query, ctx=100):
    ...
    if idx == -1:
        return body[:ctx]
    s = max(0, idx - ctx//2)
    e = min(len(body), idx + len(query) + ctx//2)
    snip = body[s:e]
    return ("..." if s>0 else "") + snip + ("..." if e<len(body) else "")
