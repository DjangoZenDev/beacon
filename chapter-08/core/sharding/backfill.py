...
"""Beacon v0.8 — Backfill (carried from Ch6)."""
import logging
    ...
    for idx, org_id in enumerate(org_ids):
        try:
            ...
            for i in range(0, len(pages), BATCH_SIZE):
                _bulk_pages(pages[i:i+BATCH_SIZE], new_shard)
                stats["pages_copied"] += len(pages[i:i+BATCH_SIZE])
            links = list(PageLink.objects.using(old_shard).filter(organization_id=org_id).values())
            for i in range(0, len(links), BATCH_SIZE):
                _bulk_links(links[i:i+BATCH_SIZE], new_shard)
                stats["links_copied"] += len(links[i:i+BATCH_SIZE])
        except Exception as exc:
            logger.error("Backfill org %d failed: %s", org_id, exc)
            stats["errors"] += 1
    return stats
...
def _bulk_links(rows, shard):
    ...
    with connections[shard].cursor() as c:
        for r in rows:
            c.execute("INSERT INTO core_pagelink (id,organization_id,source_id,target_id,created_at) VALUES (%(id)s,%(organization_id)s,%(source_id)s,%(target_id)s,%(created_at)s) ON CONFLICT DO NOTHING", r)
