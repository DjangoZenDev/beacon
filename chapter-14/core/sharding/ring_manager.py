"""Beacon v0.14 — Ring Manager."""
import json, logging, threading
from .consistent_hash import ConsistentHashRing
logger = logging.getLogger("beacon.sharding")
class RingManager:
    def __init__(self, etcd_host="localhost", etcd_port=2379): self.etcd_host = etcd_host; self.etcd_port = etcd_port; self.client = None; self.ring = ConsistentHashRing(); self.lock = threading.Lock(); self._loaded = False
    def _get_client(self):
        if self.client is None:
            try: import etcd3; self.client = etcd3.client(host=self.etcd_host, port=self.etcd_port)
    ...
    def initialize(self, shards=None):
        ...
        if shards is None: from django.conf import settings; shards = list(getattr(settings,"SHARD_MAP",{}).keys())
        if shards: 
    ...
    def _load_ring_from_etcd(self):
        ...
        with self.lock: self.ring = ConsistentHashRing(replicas=config.get("replicas",150)); [self.ring.add_shard(s["name"]) for s in config["shards"]]
