...
def notify_bookmarkers(self, page_id, edited_by_user_id, idempotency_key=None):
    from core.idempotency import check_and_claim, store_result; from core.models import Page
    ...
    if not is_new: return previous or "Duplicate"
    User = get_user_model()
    ...
    for user in bookmarked_users:
        ...
        except Exception as exc: logger.error("Notify user %s: %s", user.id, exc)
    result = {"status":"ok","notified":notification_count}; store_result(idempotency_key, result)
