...
"""
Beacon v0.13 — Views. Template views + DRF ViewSet + Elasticsearch search.

Chapter 13: Multi-region adds X-Region header to view responses for
Istio routing verification. Views are otherwise unchanged — the regional
routing happens transparently at the infrastructure layer.
"""
...
class PageListView(OrganizationRequiredMixin, LoginRequiredMixin, ListView):
    model = Page; template_name = "core/page_list.html"; context_object_name = "pages"; paginate_by = 20
    ...
    def get_queryset(self):
        ...
        if page_ids is not None:
            return Page.objects.filter(id__in=page_ids, organization_id=org_id).select_related("author").only(
                "title","slug","author","organization_id","updated_at","incoming_count","outgoing_count"
            ).order_by("-incoming_count","-updated_at")
        qs = Page.objects.filter(organization_id=org_id).select_related("author").only(
            "title","slug","author","organization_id","updated_at","incoming_count","outgoing_count"
        ).order_by("-incoming_count","-updated_at")
...
class PageSearchView(OrganizationRequiredMixin, LoginRequiredMixin, TemplateView):
    """Elasticsearch-backed search view with BM25 scoring and fuzziness=AUTO."""
    ...
    def get_context_data(self, **kwargs):
        ...
        query = self.request.GET.get("q", "").strip()
        ctx["query"] = query
        if not query: ctx["results"], ctx["total_hits"], ctx["facets"] = [], 0, []; return ctx
        try:
            ...
            response = s.execute()
            ctx["results"] = [{
                "id": hit.meta.id, "title": hit.title, "slug": hit.slug,
                "author": hit.author_username, "updated_at": hit.updated_at,
                "snippet": ("...".join(hit.meta.highlight.body)[:200] if hasattr(hit.meta, "highlight") and hit.meta.highlight else hit.body[:200]),
            } for hit in response]
            ...
            ctx["results"], ctx["total_hits"], ctx["facets"] = [], 0, []
            ctx["search_error"] = "Search is temporarily unavailable."
        return ctx
...
class PageDetailView(OrganizationRequiredMixin, LoginRequiredMixin, DetailView):
    ...
    def get_object(self, queryset=None):
        slug = self.kwargs.get("slug"); org_id = self.organization_id
...
class PageEditView(OrganizationRequiredMixin, LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            import uuid
            notify_bookmarkers.delay(page_id=self.object.pk, edited_by_user_id=self.request.user.pk,
                idempotency_key=f"notify:{self.object.pk}:{self.request.user.pk}:{uuid.uuid4().hex[:12]}")
...
class FeedView(OrganizationRequiredMixin, LoginRequiredMixin, TemplateView):
    """Chapter 11: Hybrid feed — Redis sorted sets for active users,
    event-store queries for inactive users. Carried forward to Ch13."""
