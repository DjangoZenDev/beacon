...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--batch-size", type=int, default=50); parser.add_argument("--sleep", type=float, default=0.5); parser.add_argument("--broker", choices=["redis","kafka"], default="kafka")
    ...
    def _poll_and_publish(self, batch_size, broker):
        ...
        for msg in messages:
            try:
                self._publish(msg, broker); OutboxMessage.objects.filter(pk=msg.pk).update(status=OutboxMessage.STATUS_SENT, processed_at=timezone.now())
            except Exception as exc:
                msg.refresh_from_db(); new_retry = msg.retry_count + 1
                if new_retry >= msg.max_retries: OutboxMessage.objects.filter(pk=msg.pk).update(status=OutboxMessage.STATUS_FAILED, last_error=str(exc), retry_count=new_retry); logger.error("Outbox msg %s FAILED: %s", msg.pk, exc)
                else: OutboxMessage.objects.filter(pk=msg.pk).update(status=OutboxMessage.STATUS_PENDING, last_error=str(exc), retry_count=new_retry); logger.warning("Outbox msg %s retry %d: %s", msg.pk, new_retry, exc)
        return len(messages)
