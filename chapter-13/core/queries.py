"""Beacon v0.13 — Scatter-Gather Queries."""
...
def _search_shard(shard, query, org_ids):
    ...
    try:
        pages = Page.objects.using(shard).filter(organization_id__in=org_ids).filter(
            Q(title__icontains=query)|Q(body__icontains=query)
        ).only("title","slug","organization_id","incoming_count").order_by("-incoming_count")[:50]
        return [{"title":p.title,"slug":p.slug,"org_id":p.organization_id,
                 "incoming_count":p.incoming_count,
                 "excerpt":_excerpt(p.body,query) if hasattr(p,"body") and p.body else ""} for p in pages]
