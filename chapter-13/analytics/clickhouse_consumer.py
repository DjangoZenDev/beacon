...
"""
Beacon v0.13 — ClickHouse Kafka Consumer

Consumes "beacon.events.page_views" topic from Kafka and
batch-inserts into ClickHouse for real-time analytics.

Chapter 13: In multi-region mode, each region runs its own
ClickHouse consumer. Regional analytics are federated at the
query layer via distributed tables.
"""
...
class ClickHouseIngestor:
    def __init__(self, ch_host="localhost", ch_port=9000, ch_db="beacon_analytics"):
        self.ch_host = ch_host; self.ch_port = ch_port; self.ch_db = ch_db
        self._client = None
    ...
    def client(self):
        if self._client is None:
            ...
            self._client = Client(host=self.ch_host, port=self.ch_port, database=self.ch_db)
        return self._client
...
def run_clickhouse_consumer():
    ...
    consumer = KafkaConsumer(
        ...
        auto_offset_reset="earliest", enable_auto_commit=True, max_poll_records=1000,
    )
    logger.info("ClickHouse consumer started on beacon.events.page_views")
    batch = []; batch_size = 1000; last_flush = time.time(); flush_interval = 5
    for message in consumer:
        try:
            event = message.value; payload = event.get("payload", {})
            batch.append({
            ...
            if len(batch) >= batch_size or (time.time() - last_flush) > flush_interval:
                ingestor.insert_batch(batch); batch = []; last_flush = time.time()
        except Exception as exc:
            logger.error("ClickHouse consumer error: %s", exc)
    if batch: ingestor.insert_batch(batch)
