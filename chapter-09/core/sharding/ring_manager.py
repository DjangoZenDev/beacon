...
"""Beacon v0.9 — Ring Manager (carried from Ch6)."""
    ...
    def _get_client(self):
        if self.client is None:
            try:
                ...
                logger.error("etcd3 unavailable: %s", exc)
                return None
        return self.client
    ...
    def initialize(self, shards=None):
        ...
        if client:
            try:
                ...
                logger.warning("etcd load failed: %s", exc)
        if shards is None:
    ...
    def _start_watcher(self):
        ...
        def loop():
            ...
            if c is None:
                return
            try:
                for event in c.watch("/beacon/shards/ring"):
                    cb(event)
            except Exception:
                pass
