...
"""
Beacon v0.9 — Background Tasks with Idempotency (carried from Ch8).

Chapter 8: notify_bookmarkers now uses idempotency keys to prevent
duplicate notifications from at-least-once message delivery.

Principle 3: "Make every task idempotent."
"""
...
def notify_bookmarkers(self, page_id, edited_by_user_id, idempotency_key=None):
    ...
    if idempotency_key is None:
        idempotency_key = f"notify:{page_id}:{edited_by_user_id}:{uuid.uuid4().hex[:12]}"

    is_new, previous = check_and_claim(idempotency_key)
    if not is_new:
        logger.info("Duplicate task skipped: key=%s", idempotency_key)
        return previous or "Duplicate — already processed"
    ...
    try:
        ...
        store_result(idempotency_key, {"status": "not_found"})
    ...
    logger.info("Notified %d users about page %s (key=%s)", notification_count, page_id, idempotency_key)
    return f"Notified {notification_count} users"


def send_notification(user, page):
    """Stub: send notification to a user about a page edit."""
    pass
