"""
Beacon v0.16 — Organization Shard Router. Principle 6: Shard by Tenant.

Routes reads to replicas and writes to the primary shard based on
organization_id. Uses consistent hashing for deterministic routing.
"""
...
class OrganizationShardRouter:
    ...
    def db_for_write(self, model, **hints):
        return self._resolve(model, **hints) or "default"
    ...
    def _resolve(self, model, **hints):
        ...
        ring = ConsistentHashRing(list(getattr(settings,"SHARD_MAP",{}).keys()))
        try: return ring.get_shard(org_id)
