...
"""
Beacon v0.10 — Views. Template views + DRF ViewSet + Elasticsearch search.

Chapter 10 adds PageSearchView: Elasticsearch-backed search with
multi_match (title^3, body), BM25 scoring, fuzziness=AUTO, and
...
class PageSearchView(OrganizationRequiredMixin, LoginRequiredMixin, TemplateView):
    """
    Elasticsearch-backed search view.

    Uses elasticsearch-dsl Search() DSL with multi_match on title^3
    and body, BM25 scoring, fuzziness=AUTO for typo tolerance, and
    faceted aggregations grouped by author.
    """
