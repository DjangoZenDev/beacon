"""Beacon v0.10 URL configuration.

WebSocket routes are handled by the ASGI router (beacon/asgi.py),
not by the WSGI urlpatterns. See core/routing.py for WebSocket routing.
"""

from django.contrib import admin
from django.urls import path, include
from core import views

urlpatterns = [
    path("admin/", admin.site.urls),
    path("", views.PageListView.as_view(), name="page_list"),
    path("page/<slug:slug>/", views.PageDetailView.as_view(), name="page_detail"),
    path("page/<slug:slug>/edit/", views.PageEditView.as_view(), name="page_edit"),
    path("search/", views.PageSearchView.as_view(), name="search"),
    path("api/", include("core.api_urls")),
]
