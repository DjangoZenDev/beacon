"Beacon v0.12 — Collaboration Consumer (WebSocket)."
import json, logging
...
class CollaborationConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def crdt_operation(self, event):
        crdt = await self._load_crdt()
        if event["op"] == "insert":
            remote_char = Char(char_id=event["char_id"],value=event["char"],parent_id=event["after_id"],client_id=event["client_id"],lamport_ts=event["lamport_ts"])
    ...
    async def _save_crdt(self, crdt):
        ...
        serialized = {char_id: (c.value, c.parent_id, c.client_id, c.lamport_ts) for char_id, c in crdt.chars.items()}
